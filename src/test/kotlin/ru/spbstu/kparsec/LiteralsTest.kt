...
class LiteralsTest {
    ...
    fun `string literals should work`() {
        Literals.JSTRING.parse("abcdef").assertFail()
        Literals.JSTRING.parse("\"abcdef").assertFail()
        Literals.JSTRING.parse("abcdef\"").assertFail()
        assertEquals("", Literals.JSTRING.parse("\"\"").assertResult())
        assertEquals("abcdef", Literals.JSTRING.parse("\"abcdef\"").assertResult())
        assertEquals("abc", Literals.JSTRING.parse("\"abc\"def\"").assertResult())
        assertEquals("\u00ff", Literals.JSTRING.parse("\"\\u00ff\"").assertResult())
        assertEquals("\u00ff", Literals.JSTRING.parse("\"\\xff\"").assertResult())
        Literals.JSTRING.parse("\"\\u0ff\"").assertFail()
        assertEquals("\n\t\t", Literals.JSTRING.parse("\"\\n\\t\\t\"").assertResult())
        assertEquals(0.toChar().toString(), Literals.JSTRING.parse("\"\\0\"").assertResult())
        assertEquals(40.toChar().toString(), Literals.JSTRING.parse("\"\\50\"").assertResult())
        assertEquals(40.toChar().toString(), Literals.JSTRING.parse("\"\\050\"").assertResult())
        assertEquals(40.toChar().toString() + "0", Literals.JSTRING.parse("\"\\0500\"").assertResult())
        assertEquals("aaa\"aaa", Literals.JSTRING.parse("\"aaa\\\"aaa\"").assertResult())
    }
}
